...
// import './MaintenanceBackground.css';
...
float pattern(vec2 p) {
  vec2 p2 = p - time * waveSpeed * flowDirection + vec2(randomSeed);
  return fbm(p - fbm(p + fbm(p2)));
}
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    enableMouseInteraction: new THREE.Uniform(enableMouseInteraction ? 1 : 0),
    mouseRadius: new THREE.Uniform(mouseRadius),
    randomSeed: new THREE.Uniform(Math.random() * 1000),
    flowDirection: new THREE.Uniform(new THREE.Vector2(Math.random() * 2 - 1, Math.random() * 2 - 1).normalize()),
  });
