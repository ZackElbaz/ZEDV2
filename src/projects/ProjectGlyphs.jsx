...
function ProjectGlyphs() {
  ...
  const processAndCropGlyphs = async (files) => {
        ...
        ({ file, index }) =>
          new Promise((resolve) => {
            ...
            image.onload = () => {
              const side = Math.min(image.width, image.height);
              const canvas = document.createElement("canvas");
              const GLYPH_SIZE = 512; // or 256 if you want crisp AF
              ...
              const [avgR, avgG, avgB] = getAverageRGB(ctx, GLYPH_SIZE, GLYPH_SIZE);
    ...
    setGlyphAvgColors(realGlyphs.map((_, i) => realIntensities[i]));
    ...
    while (paddedGlyphs.length < fullCount) {
      ...
      const dummyUrl = dummyCanvas.toDataURL("image/png");
      paddedGlyphs.push({ id: -1, url: dummyUrl, blob: null, loading: false });
      paddedAvgColors.push([999, 999, 999]);
    }
    ...
    setGlyphAtlas(flippedCanvas);
    setGlyphImages(realGlyphs);
    setGlyphAvgColors(realAvgColors);
    setNeedsUpdate(true);
  ...
  }, [selectedDeviceId, videoDevices, isFrontFacing]);
  ...
  const intro = [
    ...
    },
    {
      ...
      content: (
        <>
          This program creates photomosaic images by replacing square sections
          (kernels) of input media with smaller 'glyph' images. Each kernel is
          matched to the glyph that best fits it—either by comparing color (Red,
          Green, and Blue values) or brightness (Greyscale Intensity). When
          matching by color, the glyph with the closest average color to the
          kernel is selected. When matching by intensity, only brightness is
          considered, and the glyph with a similar lightness or darkness is
          chosen. This creates an overall image that retains its structure but
          is built entirely out of glyphs.
        </>
      ),
    },
    {
  ...
  const topControls = [
    {
      label: "🗂️ Media File from Computer",
      kind: "file",
      accept: "image/*,video/*",
      onChange: onFileChange,
    },
    {
      label: "📂 Upload Glyph Folder",
      kind: "file",
      accept: "image/*",
      multiple: true,
  ...
  const canvasBlock = (
    <div className="template-canvas-shell">
      <canvas
        ref={canvasRef}
        className="template-project-canvas"
        style={{ backgroundColor: "#000" }}
      />
    </div>
  );

  const bottomControls = glyphAtlasReady
    ? [
        {
          label: showGlyphPreview ? "Hide Glyphs" : "View Glyphs",
          onClick: () => setShowGlyphPreview((prev) => !prev),
        },
        {
          label: `Matching by: ${matchByIntensity ? "Intensity" : "Color"}`,
          onClick: () => {
  ...
  const afterSliders = (
    <div style={{ marginTop: "2rem" }}>
      ...
      {glyphAtlasReady && showGlyphPreview && glyphImages.length > 0 && (
        <div
          style={{
            display: "grid",
