...
import initVoronoiStippling from "./ProjectVoronoiShader";

const ENABLE_POINT_CSV_EXPORT = false;

function createDefaultConvState() {
  return {
    percent: 0,
    normStep: Infinity,
    capRms: Infinity,
...
export default function ProjectVoronoi() {
  ...
  const [numPoints, setNumPoints] = useState(1200);
  const [contrast, setContrast] = useState(1.0);

  const [conv, setConv] = useState(createDefaultConvState());
  ...
  useEffect(() => {
    ...
    simRef.current = initVoronoiStippling(canvas, {
      ...
      onConvergence: (s) => {
        setConv({
          percent: s.percent ?? 0,
          normStep: s.normStep ?? Infinity,
          capRms: s.capRms ?? Infinity,
          threshold: s.threshold ?? 0.0015,
          capThreshold: s.capThreshold ?? 0.06,
          done: !!s.done,
      ...
      onUiStateChange: (ui) => {
        setIsRunning(!!ui.isRunning);
        setIsLiveSource(!!ui.isLiveSource);
        setShowImage(!!ui.showImage);
      },
    });

    simRef.current.loadDefaultImage(`${process.env.PUBLIC_URL || ""}/Giraffe.png`);
  ...
  function handleImageFile(e) {
    const file = e.target.files?.[0];
    if (!file) return;
    simRef.current?.loadImageFile(file);
    e.target.value = "";
    setConv(createDefaultConvState());
  }

  function handleVideoFile(e) {
    const file = e.target.files?.[0];
    if (!file) return;
    simRef.current?.loadVideoFile(file);
    e.target.value = "";
    setConv(createDefaultConvState());
  }

  function handleLiveCamera() {
    simRef.current?.startCamera();
    setConv(createDefaultConvState());
  }

  function handleRunPause() {
    simRef.current?.toggleRun();
  }
  ...
  function handleToggleImage() {
    simRef.current?.toggleImage();
  }

  function handlePointsChange(e) {
    const n = Number(e.target.value);
    setNumPoints(n);
    simRef.current?.setPointCount(n);
    setConv(createDefaultConvState());
  }
  ...
  function handleContrastChange(e) {
    ...
    setContrast(v);
    simRef.current?.setContrast(v);
    setConv(createDefaultConvState());
  }

  function handleExportCsv() {
    simRef.current?.exportPointsCsv();
  }

  const percentLabel = conv.done
    ? "100%"
    : `${Math.round((conv.percent || 0) * 100)}%`;
  ...
  const intro = [
    {
      ...
      content: (
        <>
          <a
            href="https://en.wikipedia.org/wiki/Stippling"
            target="_blank"
            rel="noreferrer"
          >
            Stippling
          </a>{" "}
          is drawing with tiny dots. To make shadows, artists simply pack more
          dots into darker areas and leave highlights almost empty. Along curves
          and edges they’ll sometimes lay down little chains of dots to echo the
          form and keep everything feeling smooth and alive.
        </>
      ),
    },
    {
  ...
  const sections = [
    {
      ...
      paragraphs: [
        <>
          ...
          . Then we nudge each dot toward the center of its patch and repeat.
          This gentle nudge-and-repeat step is called{" "}
          <a
            href="https://en.wikipedia.org/wiki/Lloyd%27s_algorithm"
            target="_blank"
            rel="noreferrer"
          >
            relaxation
        ...
        <>
          ...
          : the plain middle of the cell.
          <br />
          •{" "}
          <a
            href="https://en.wikipedia.org/wiki/Center_of_mass"
            target="_blank"
            rel="noreferrer"
          >
            Centre of mass
          </a>
          : the darkness-weighted middle, so darker pixels pull harder. Think of
          it like a hammer, the geometric centre isn’t the same as the centre of
          mass. Using weighted centres makes dots drift into shadows, exactly
          what an artist would do by hand.
        </>,
        <>
          This patchwork isn’t just a computer thing, you see similar tiling in
          nature, like the blocky mosaics on a{" "}
          <a
            href="https://en.wikipedia.org/wiki/Giraffe#Species_and_subspecies"
            target="_blank"
            rel="noreferrer"
          ...
          . Different processes make those patterns, but the look is very
          Voronoi-ish.
        </>,
      ],
      media: [
    ...
    {
      title: "Why hexagons keep popping up",
      paragraphs: [
        <>
          Keep relaxing and the dots start arranging themselves like a{" "}
          <a
            href="https://en.wikipedia.org/wiki/Crystal"
            target="_blank"
            rel="noreferrer"
          >
            crystal
          </a>
          . Locally, the patches want to be as even as possible, so the dots
          settle into a hexagonal packing, the same way soap bubbles and real
          crystals often organize when they’re minimising “energy”. Don’t worry
          though: it’s just the universe whispering, “hexagons are efficient.”
        </>,
        <>
          If you like satisfying packing stories, Veritasium’s video on{" "}
          <a
          ...
          shows how millions of spheres self-arrange on water, another glimpse
          of order emerging from simple rules.
        </>,
      ],
      media: [
        {
    ...
    {
      title: "Capacity-Constrained: cleaner dots, fewer artifacts",
      paragraphs: [
        <>
          Pure relaxation tends to over-crystallize (hello, hexagons).
          <a
          ...
          fixes that by giving every dot the same amount of image mass to “own”.
          Dark pixels weigh more than light ones. Instead of equal areas, each
          dot balances equal capacity. The result keeps dots concentrated where
          the image is dark, without locking into a rigid honeycomb.
        </>,
        <>
          That’s why CCPD ends up looking like{" "}
          <a
            href="https://en.wikipedia.org/wiki/Colors_of_noise#Blue_noise"
            target="_blank"
            rel="noreferrer"
          >
            blue noise
          </a>{" "}
          (dots that feel evenly sprinkled, with no clumps and no obvious grid).
          Our eyes are very good at spotting stripes, bands, or checkerboard
          patterns, which can look distracting (that’s the “moiré” effect).
          Blue-noise spreads tiny differences around so nothing lines up into
          bands, and the texture fades into the picture causing you to just see
          smooth shading. Think of it like “salt shaken evenly over a surface”:
          you don’t notice the grains, only the even tone.
        </>,
      ],
    },
  ];
  ...
  const buttons = [
    ...
    {
      label: showImage ? "Show Points" : "Show Image",
      onClick: handleToggleImage,
      active: showImage,
    },
    ...(ENABLE_POINT_CSV_EXPORT
      ? [
          {
  ...
  const progress = {
    label: "Converged:",
    percent: conv.done ? 100 : Math.round((conv.percent || 0) * 100),
    valueLabel: percentLabel,
  };

  // const status = [
  //   <strong key="status-title">Status</strong>,
  //   `Iteration: ${conv.iteration}`,
  //   `Normalised movement: ${
  //     Number.isFinite(conv.normStep) ? conv.normStep.toFixed(5) : "—"
  //   }`,
  //   `Capacity RMS: ${
  //     Number.isFinite(conv.capRms) ? conv.capRms.toFixed(5) : "—"
  //   }`,
  //   `Point radius: ${
  //     simRef.current?.getPointRadius
  //       ? simRef.current.getPointRadius().toFixed(2)
  //       : "—"
  //   }`,
  //   conv.done && !isLiveSource
  //     ? "Simulation stopped at convergence"
  //     : isRunning
  //     ? "Running"
  //     : "Paused",
  // ];
  ...
  return (
    <ProjectTemplate
      title="Dots, Cells & Giraffes: A Tiny Tour of Stippling"
      intro={intro}
      sections={sections}
