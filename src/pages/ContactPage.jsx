...
function ContactPage() {
  ...
  // Set dynamic blob color on mount
  useEffect(() => {
    const getRandomVibrantNeonColor = () => {
    ...
    document.documentElement.style.setProperty("--blob-color", blobColor);
    document.documentElement.style.setProperty("--hover-highlight", inverseColor);
  }, []);
  ...
  // Touch swipe snapping in portrait mode
  useEffect(() => {
    if (!isPortrait || sectionHeight === 0) return;
    const container = layoutRef.current;
    if (!container) return;
    ...
    const scrollToSection = (index) => {
      ...
      const checkScrollSettled = () => {
        if (Math.abs(container.scrollTop - targetOffset) < 2) {
          isSnapping = false;
        } else {
          requestAnimationFrame(checkScrollSettled);
        }
